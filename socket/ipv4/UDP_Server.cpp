...
fromlen传入的参数必须足够大来保存from，函数结束后fromlen被设置为from中有效数据的大小
flags常用参数（未完全列举）
...
int main(int argc, char* argv[])
{
    ...
    writeLen = sendto(server_fd, buf, strlen(buf), 0, (struct sockaddr*)&client_addr, sizeof(client_addr));
    printf("writeLen = %d\n", writeLen);
    if(writeLen < 0)
    {
